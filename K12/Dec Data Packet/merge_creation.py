...
ccd_school_counts = data_pull.ccd_counts_cleaned(debug=True)
ccd_counts_merge = ccd_counts_merge.merge(ccd_school_counts, on='state_abrv', how = 'outer')
ccd_counts_merge["student_teacher_ratio"] = ccd_counts_merge['student_teacher_ratio'].apply(lambda x: f"{x:,.2f}" if pd.notna(x) else x)
...
print(ccd_all_merge.to_string())
#save to merge dict
merge_files['ccd'] = ccd_all_merge
    # return ccd_all_merge
# %% SAT Data  ++++++++++++++++++++
from data_sources.data_collection import sat_data
...
# %% ACT DATA ++++++++++++++++ comeback
from data_sources.data_collection import act_data_2024
...
chron_abs_race_df = chron_abs_race_df.loc[:,['state', 'updated data year']]
...
chron_abs_other_df = chron_abs_other_df.loc[:,['state', 'updated data year']]
